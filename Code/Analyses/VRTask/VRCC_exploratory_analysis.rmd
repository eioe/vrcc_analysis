...
2. Comparison of localization error between cardiac phases [S2](#S2)
3. RR interval length and distance estimates [S3](#S3)
4. Animal assessment - threat, disgust, and speed ratings  [S4](#S4)
5. Distance error as a function of subjective threat [S5](#S5)
6. Distance error as a function of anxiety levels, digust, and speed ratings [S6](#S6)
7. Cybersickness (SSQ) and presence in VR (SUS) [S7](#S7)
...
d_sub_start <- read.csv(file = paste0(VRCC_dir,"/Data/VRTask/VRCC_data_initial_subjective_ratings.csv"))

# read mid-experimental ratings data
d_sub_mid <- read.csv(file = paste0(VRCC_dir,"/Data/VRTask/VRCC_data_mid-experimental_subjective_ratings.csv"))
d_sub_mid$threat[d_sub_mid$threat == 0] <- NA # add NAs

# read questionnaires data
d_qstn <- read.csv(file = paste0(VRCC_dir,"/Data/VRTask/VRCC_data_questionnaires.csv"))
...
# TODO: fix this [copied from main analysis ]
